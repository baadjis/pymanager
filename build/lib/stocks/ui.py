...
from ta import candelistick, lrc,moving_average,rsi
from dataprovider import yahoo
from index import Index
from nlp import compute_cosine_similarity,get_named_entities,get_lemmatize
...
def execute_func(func:str,ticker,stock:Stock):
    ...
    if func=="infos":
        infos=stock.infos
        longbus=infos.pop('longBusinessSummary')
        st.write(longbus)
    ...
    if func=="news":
        ...
        for n in news: 
            ...
            st.subheader(n["content"]["title"])
            st.text(n["content"]["summary"])
            st.link_button("Read more", n["content"]["clickThroughUrl"]["url"])
            st.divider()
    ...
    if func=="chart":
        ...
        if graph_select=="plot":
            ...
            if plot_type=="candlestick":
                ...
                candelistick(ax=plot_price,data=data)
...
def explore_stock():
    ...
    if ticker:
        ...
        value=np.round(close[-1],2)[0]
        print(value)
        pch=np.round((close[-1]-close[-2])/close[-2],2)[0]
        delta=np.round(close[-1]-close[-2],2)[0]
...
def screen_stocks():
    json_data=dask_read_json("dataprovider/jsons/major_indices.json")
   

    wiki_indexes=dask_read_json("dataprovider/jsons/wiki_indexes.json")
    print(wiki_indexes)
    regions=wiki_indexes["region"].dropna().unique()
    countries=wiki_indexes["country"].dropna().unique()
    print(countries)
    indexes=json_data["Symbol"].unique()
    sectors_list=["consumer","telecom"]
    industries_list=["consumer","telecom"]
    global_indexes=wiki_indexes[wiki_indexes.geo=="global"].name
    st.session_state.filters_dicts=[]
    if "theme" not in st.session_state:
        st.session_state.theme="major"
...
def deepbot():
    ...
    def get_most_similar(l_words,question):
        return sorted(l_words,key=lambda x:compute_cosine_similarity(question,x),reverse=True)[0]
    def get_nlp(q):
        quest_dict={}
        tockens=q.split()
        for t in tockens:
    ...
    for q in st.session_state.questions:
            ...
            named_entities=get_named_entities(q)
            get_nlp(q)
