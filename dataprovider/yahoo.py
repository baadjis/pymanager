...
def retrieve_data(tickers: tuple[str],period='5y'):
    print(tickers)
    history = yf.download(tickers=list(tickers),period=period,auto_adjust=False)
    return history
@lru_cache()
def get_ticker_data(ticker_name:str,period='5y'):
  if period=="1d" or period=="5d":
     return yf.download(ticker_name,period=period,interval="1h",auto_adjust=False)
  data= yf.download(ticker_name,period=period,auto_adjust=False)
...
def get_ticker_dividends(ticker):
    st=yf.Ticker(ticker=ticker)
    div = st.get_dividends()
   
    return div
    
def get_balance_sheet(ticker):
    st=yf.Ticker(ticker=ticker)
    div = st.get_balancesheet()
